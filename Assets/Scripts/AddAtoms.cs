...
public class AddAtoms : MonoBehaviour
{
    ...
    /* public List<Vector3> tempList = new List<Vector3>();
     private List<Vector3> proteinVec = new List<Vector3>();
     public List<OBAtom> proteinAt = new List<OBAtom>();
     public List<bool> proteinResAt = new List<bool>();
     public List<GameObject> atomPointers = new List<GameObject>();
     public Collider test;
     public GameObject temp;
     AudioSource audioSource;*/
    public List<int[]> proteinRes = new List<int[]>();
    public List<int> resIDS = new List<int>();
    //for atom numbering
    public int num = 0;
    ...
    public void parsePDB()
    {
        ...
        List<Vector3> oBondAcceptors = new List<Vector3>();
        List<Vector3> oBondDonors = new List<Vector3>();

        //For collecting protein within a certain radius of the ligand
       // List<Vector3> ligandVec = new List<Vector3>();
        // List<Vector3> proteinVec = new List<Vector3>();
       // VectorpAtom test = new VectorpAtom();
    ...
    parseMol: // iteration start if multiple input files are used 
        ...
        for (int i = 1; i < numAtoms; i++)
        {
            ...
            //Below are comments and a short discussion on a simple code procedure to display the protein residues within 5 angstroms of a (*"the" for the time being) ligand

            //check to see if its a ligand atom.
            //if ligand atom collides with 4.5 angstrom radius display that protein residue
            //if not skip it.
            //So the way they seem to have it is simple:
            /* First they grab both the elem and position and place it in the atoms list.
            Then through a sequence of if statements, it checks to to see what it should be displayed as and changes the tag
            as neccessary. Said tag is used to determine what will be displayed. In our case what we should do is this:
            Let the code keep doing what it's doing except when it checks for the ligand atom we check for the surrounding atoms. This may be easily done
            using sphere colliders; must look into further. 
            Problem here is if we use Sphere Colliders we would have to wait for the whole structure to display first and since we intend to make this a selectable option within the menu,
            its probably better to do this (OR AT THE VERY LEAST ANALYZE AFTERWARD)*/
            /* psuedo code

            for(each atom in ligand)
            for(each atom in protein residue)
                if(there is a protein within 5 angstroms & is not already in list)
                   //use display function similar to its usage below
                   //displayTEST(protein)
             */
            int n;
            ...
            { //ligand
                ...
                { //ion (should be 3 but a bug in openbabel labels ions as protein, thats why the check has to be done inside the ligand check)
                    ...
                    /*What I want to do in these code segments is add other interations such as between Na and O
                     And Cl and H
                    The former needing to be within 3 angstroms of each other and the latter being within 2 angstroms.
                    I would like to set it up similarly to the way they have the hBond's set up however it looks like the way its going to be set up is like so:

                    First lets find the H atoms near the Cl atoms (within 2 angstroms)
                    Do the same for Na and O atoms except for 3 angstroms
                    Once a set is found show the bond by using the code they have for checkHBond. Not neccessarily sure how it works but from the looks of it
                    it seems to be similar to how I tried to determine binds between two atoms in my program lets test that out tommorow and 
                ...
                if (atom.HasResidue())
                {
                    ...
                    int[] resAtoms = new int [residue.GetNumAtoms()];
                    resIDS.Add((int)residue.GetIdx());
                    for (int k = 0; k < resAtoms.Length; k++)
                    {
                        resAtoms[k] = (int)residue.GetAtoms()[k].GetIndex();
                        //Debug.Log((int)residue.GetIdx() + "  " + resAtoms[k]);
                        
                    }
                    proteinRes.Add(resAtoms);
            ...
            //atoms[i].transform.position = new Vector3((float)atom.GetX(), (float)atom.GetY(), (float)atom.GetZ());
            centerPos += position;
            centerCount++;
            //atomPointers.Add(atoms[i]);

        }
        //createSurface (atomPositions,atomElements); 
        Debug.Log(hBondAcceptors.Count + " " + hBondDonors.Count);
        /******************************************************************************************************/
        //REALLY REALLY REALLY REALLY REALLY SLOPPY AND SLOW MUST FIX LATER. HAIL LAZINESS AND REFUSAL TO THINK
        /*
        if (ligandVec.Count != 0 & proteinVec.Count != 0)// & loopCount > 0)
        {
            List<uint> residueNums = new List<uint>();
            for (int i = 0; i < ligandVec.Count; i++)
            {
                ...
                {
                    float dis = 5;
                    //Debug.Log(proteinVec[k]);
                    if (Vector3.Distance(ligandVec[i], proteinVec[k]) <= dis & proteinResAt[k] == true & !residueNums.Contains(proteinAt[k].GetResidue().GetNum()))
                    {

                        test = (proteinAt[k].GetResidue().GetAtoms());
                        residueNums.Add(proteinAt[k].GetResidue().GetNum());
                        for (int j = 0; j < test.Count; j++)
                        {
                            Vector3 pos1 = new Vector3((float)test[j].GetX(), (float)test[j].GetY(), (float)test[j].GetZ());
                            for (int s = 0; s < atomPointers.Count; s++)
                            {
                                if (pos1 == atomPointers[s].transform.position)
                                {
                                    atoms[s].transform.tag = "stuff";
                                }
                            }
                        }
    ...
    void connect(GameObject atom1, GameObject atom2, OBBond bondInfo)
    {
        ...
        if (atom1.transform.tag == "hetatmbs")
            tag = "hetatms";
        else if (atom1.transform.tag == "water" | atom1.transform.tag == "stuff")
            return;
        else
            //Debug.Log(atom1.transform.tag);
            tag = "bonds";
    ...
    public void show()
    {
        ...
        /************************************************************************************************/
        /**************************************************/
        /* HERES IS WHERE OUR AUDIO TESTING STARTS!!!!!!!!!
        /**************************************************/
        /*If I find a game object with the tag "hetatmbs" that mean that we will play "brian" audio once the Player gets within 5 angstroms of 
        the ligand */
    }
    /* created the Update function (the function that Unity calls every frame) to check to see if the camera position is
    near the SphereCollider. Then play the sound "brian"
    Unfortunatly we keep getting a nullpointerexception when referencing a GameObject with the tag "bonds"
    */

    /**************************************************/
    /* HERES IS WHERE OUR AUDIO TESTING ENDS!!!!!!!!!!!
    /**************************************************/
    /************************************************************************************************/
    ...
    public void toggleDisplayMode(string tag, bool toggle)
    {
        ...
        /*********************************************************************************/
        /*else if (tag == "stuff" & gameObjects.Length != 0)
        {
            Debug.Log("Hey it passed");
            Debug.Log(tag + " heres the tag");
    ...
    //O-Na interations
    /*****************/
    /* private void checkO_NABOND(Vector3 position1, Vector3 position2)
     {
         if (Vector3.Distance(position1, position2) < 3.0f)
         { //we have a OBond
             GameObject go = createObject(hBond, position2);
             go.tag = "obond";
             go.transform.LookAt(position1);
             go = createObject(oBond, position1);
             go.tag = "obond";
             go.transform.LookAt(position2);
         }
